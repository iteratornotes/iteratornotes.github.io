...
const importpearloption = document.getElementById('importpearloption');
const addpearloption = document.getElementById('addpearloption');
...
document.addEventListener('contextmenu', (e) => {
    ...
    if (pearlclicked || categoryclicked) {
        ...
        clampMenu(pearlcontextmenu, e.clientX + window.scrollX, e.clientY + window.scrollY);
        backgroundcontextmenu.style.display = 'none';
    } else {
        pearlcontextmenu.style.display = 'none';

        const anymodalon = settingsmodalon || pearlmodalon || editpearlmodalon;
        const oninteractive = e.target.closest('#centerglyph') || e.target.closest('.rectbutton') || e.target.closest('.circlebutton') || e.target.closest('.texteditor') || e.target.closest('input');
        if (!anymodalon && !oninteractive) {
            e.preventDefault();
            backgroundcontextmenu.style.display = 'flex';
            clampMenu(backgroundcontextmenu, e.clientX + window.scrollX, e.clientY + window.scrollY);
        } else {
            backgroundcontextmenu.style.display = 'none';
        }
    }
...
texteditor.addEventListener('contextmenu', function(e) {
    ...
    if (selectedtext.length > 0) {
        ...
        clampMenu(textcontextmenu, e.clientX, e.clientY);
    }
});
...
addbutton.addEventListener('touchend', function(e) {
    e.preventDefault(); // prevents the delayed mouse click from also firing
    if (addbutton.style.pointerEvents === "none") return;
    pearlmodalon = false;
...
let editpearlmodalon = false;
let selectedadd = "pearl";
let introPlayed = false;
let introskipready = false;
let introtimeouts = [];
let allpearls = [];
let layer = "top";
...
let firstvisit = localStorage.getItem('introplay') === null;
let introplay = localStorage.getItem('introplay');
if (introplay === null) {
    introplay = false;
    localStorage.setItem('introplay', 'false');
...
document.addEventListener('click', (e) => {
    ...
    if (pearlclicked) {
        ...
        editpearlmodalon = true;
        glyphtitle.style.setProperty('--color', pearl.color);

        glyphtitle.textContent = pearl.name.slice(0, 10)
    ...
    if (e.target.closest('#renameoption')) {
        if (contextid) {
            const item = contexttype === "pearl"
                ...
                : findcategory(allpearls, contextid);
            if (item) {
                const newname = prompt("Rename (must be length < 20):", item.name);
                if (newname && newname.trim() !== "") {
                    item.name = newname.trim().slice(0, 20);
                    localStorage.setItem('allpearls', JSON.stringify(allpearls));
                    updatepearls(layer);
                }
            }
    ...
    if (e.target.closest('#exportoption')) {
        if (contextid) {
            const item = contexttype === "pearl"
                ? findpearl(allpearls, contextid)
                : findcategory(allpearls, contextid);
            if (item) {
                ...
                const ext = contexttype === "pearl" ? ".pearl" : ".category";
    ...
    if (e.target.closest('#importpearloption')) {
        backgroundcontextmenu.style.display = 'none';
        const input = document.createElement('input');
        input.type = 'file';
        input.accept = '.pearl,.category';
        input.multiple = true;
        input.addEventListener('change', async (e) => {
            ...
            for (const file of files) {
                ...
                try {
                    ...
                    if (layer === 'top') {
                        allpearls.push(parsed);
                    } else {
                        const parent = findcategory(allpearls, layer);
                        if (parent) {
    ...
    if (e.target.closest('#addpearloption')) {
        backgroundcontextmenu.style.display = 'none';
        pearlmodalon = true;
        selectedadd = "pearl";
        maincontent.style.display = 'none';
    ...
    if (e.target.closest('#addcategoryoption')) {
        backgroundcontextmenu.style.display = 'none';
        pearlmodalon = true;
    ...
    if (e.target.closest('#exportbutton')) {
        const serialized = JSON.stringify(allpearls, null, 2);
        const blob = new Blob([serialized], { type: "application/octet-stream" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        ...
        if (filename) {
            a.download = filename + ".pearls";
            a.click();
            URL.revokeObjectURL(url);
        }
    }
    if (e.target.closest('#importbutton')) {
        const input = document.createElement('input');
        input.type = 'file';
        input.accept = '.pearls,.pearl,.category';
        input.multiple = true;
    ...
    if (e.target.closest('#deleteallbutton')) {
    ...
    if (intro && intro.isConnected && !introskipready) {
        introskipready = true;
...
function entermaincontent() {
    introskipready = false;
    introtimeouts = [];
    const skiptext = document.getElementById('skiptext');
    if (skiptext) skiptext.remove();
    intro.remove();
    const anymodalon = settingsmodalon || pearlmodalon || editpearlmodalon;
    if (maincontent && !anymodalon) {
...
function skipintro() {
    ...
    introtimeouts = [];
    fadevolume(rain, rain.volume, 0, 500, () => rain.pause());
    const skiptext = document.getElementById('skiptext');
    if (skiptext) skiptext.remove();
    entermaincontent();
}
    ...
    fadevolume(rain, 0,1 * audiovol ,2500, () => {
        ...
        introtimeouts.push(setTimeout(() => {
            ...
            introtimeouts.push(setTimeout(() => {
                ...
                introtimeouts.push(setTimeout(entermaincontent, 3000));
            }, 6000));
        }, 3000));
    });
}
...
function r() {
    //so radius isn't messed up on mobile
    return window.innerWidth <= 700 ? 0.420 : 0.15;
}
...
function animatepearls() {
    document.querySelectorAll(".pearl, .category").forEach(pearl => {
        ...
        if (contextid && pearl.id === contextid && pearlcontextmenu.style.display === 'flex') {
            ...
            clampMenu(pearlcontextmenu, x, y + rect.height / 2);
        }
    });
